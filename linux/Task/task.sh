...
if [ ! -d $mainHome ]
then
	echo "Cannot find the program folder, please rerun the installer script."
	exit 1
fi

if [ ! -f $extensionPath ]
then
	error "Cannot find extensions.txt"
	exit 1
fi

if [ ! -f $ignorePath ]
then
	error "Cannot find ignores.txt"
	exit 1
fi
		...
		*)
			...
			then
				...
				then
					...
					for e in ${list[@]};do
						...
						then
							...
							then
								...
								echo "- Average line count: " $lineAvg >> "$exportDir"
								echo "- All characters: " $charSum >> "$exportDir"
								echo "- Average character count: " $charAvg >> "$exportDir"
								echo >> $exportDir
							fi
						else
					...
					value "Your project average line count is " $allLineAvg
					allCharAvg=$(bc <<< "scale=2;$allCharSum/$fileCount")
					value "Your project average character count is " $allCharAvg
					...
					then
						...
						echo "Average lines / files: " $allLineAvg >> "$exportDir"
						echo "Average characters / files: " $allCharAvg >> "$exportDir"
						echo >> "$exportDir"
					fi
